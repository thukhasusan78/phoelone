...
void Otto::Home(bool hands_down) {
    if (is_otto_resting_ == false) {  // Go to rest position only if necessary
        // Prepare rest-position targets for all servos
        int homes[SERVO_COUNT];
        int max_delta = 0;
        for (int i = 0; i < SERVO_COUNT; i++) {
            if (i == LEFT_HAND || i == RIGHT_HAND) {
                if (hands_down) {
                    // Lower hands to the default rest pose
                    if (i == LEFT_HAND) {
                        homes[i] = HAND_HOME_POSITION;
                    } else {                                  // RIGHT_HAND
                        homes[i] = 180 - HAND_HOME_POSITION;  // Mirrored right-hand pose
                    }
                } else {
                    // Keep current hand positions
                    homes[i] = servo_[i].GetPosition();
                }
            } else {
                // Always home the legs and feet
                homes[i] = 90;
            }
        ...
        // Scale homing time with travel so the last motion does not stop abruptly
