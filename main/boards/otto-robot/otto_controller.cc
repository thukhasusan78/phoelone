                            ...
                            for (int i = 0; i < array_size; i++) {
                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        // Center angles (short key "o"), default 90 deg (absolute 0-180)
                                        for (int j = 0; j < SERVO_COUNT; j++) {
                                        ...
                                        // Safety: do not let both left and right legs/feet oscillate with large amplitude
                                        const int LARGE_AMPLITUDE_THRESHOLD = 40;  // Large-amplitude threshold: 40 deg
                                        ...
                                        // Phase offsets (short key "ph", degrees converted to radians)
                                        ...
                                        if (cJSON_IsNumber(period_item)) {
                                            ...
                                            if (period > 3000) period = 3000;  // Cap at 3000 ms to match the tool description
                                        }
                                        ...
                                        if (cJSON_IsNumber(steps_item)) {
                                            ...
                                            if (steps > 20.0) steps = 20.0;  // Cap at 20.0 to match the tool description
                                        }
...
public:
    ...
    void RegisterMcpTools() {
        ...
        // Phoe Lone extras. Otto no-camera I2C is GPIO_NUM_NC, so these
        // return immediately and never touch I2C, audio, or the display task.
        // Wire MPU6050 / light / touch only after free GPIOs are chosen.
        mcp_server.AddTool(
            "self.phoe_lone.imu.get_reading",
            "Return MPU6050 accel/gyro JSON. Unwired until I2C pins are assigned.",
            PropertyList(),
            [](const PropertyList& properties) -> ReturnValue {
        ...
        mcp_server.AddTool(
            ...
            "Return touch/contact state. Unwired until a free GPIO/touch pad is assigned.",
            PropertyList(),
            [](const PropertyList& properties) -> ReturnValue {
