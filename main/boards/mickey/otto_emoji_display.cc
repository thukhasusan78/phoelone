...
void OttoEmojiDisplay::SetupUI() {
    ...
    // Switch theme after UI objects are created
...
void OttoEmojiDisplay::InitializeOttoEmojis() {
    ESP_LOGI(TAG, "Otto emoji init is handled by the assets system");
    // Emoji init is handled by the assets system via DEFAULT_EMOJI_COLLECTION=otto-gif
    // assets.cc loads GIF emojis from the assets partition and applies them to the theme
...
void OttoEmojiDisplay::SetStatus(const char* status) {
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        ...
        lv_obj_set_style_text_font(status_label_, &OTTO_ICON_FONT, 0);
        lv_label_set_text(status_label_, "\xEF\x83\x81");  // U+F0c1 link icon
        lv_obj_clear_flag(status_label_, LV_OBJ_FLAG_HIDDEN);
        return;
...
void OttoEmojiDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    ...
    // Set image source and show the preview
