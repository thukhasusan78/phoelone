...
class PowerManager {
private:
    // Battery ADC range; voltage divider is two 100k resistors
    ...
    void CheckBatteryStatus() {
      // Skip ADC updates while a motion is in progress
...
public:
    PowerManager(gpio_num_t charging_pin, adc_unit_t adc_unit = ADC_UNIT_2,
      ...
      if (charging_pin_ != GPIO_NUM_NC) {
        ...
        gpio_config(&io_conf);
        ESP_LOGI("PowerManager", "Charge-detect pin configured: GPIO%d", charging_pin_);
      } else {
        ESP_LOGI("PowerManager", "Charge-detect pin not configured; skipping charge sensing");
      }
