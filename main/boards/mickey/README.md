...
> **我的身份**：
> 我是一个可爱的双足机器人Mickey，拥有四个舵机控制的肢体（左腿、右腿、左脚、右脚），没有手部舵机。
> 
> **我的动作能力**：
> - **基础移动**: 行走(前后), 转向(左右), 跳跃
...
| MCP工具名称 | 描述 | 参数说明 |
|-----------|------|---------|
| self.otto.action | 执行机器人动作 | **action**: 动作名称（必填）<br>**steps**: 动作步数(1-100，默认3)<br>**speed**: 动作速度(100-3000，数值越小越快，默认700)<br>**direction**: 方向参数(1/-1，默认1，根据动作类型不同含义不同)<br>**amount**: 动作幅度(0-170，默认30) |

#### 支持的动作列表

**基础移动动作**：
- `walk` - 行走（需 steps/speed/direction）
- `turn` - 转身（需 steps/speed/direction）
...
Mickey 没有手部舵机。不要调用举手、挥手或手臂相关动作。
...
| self.otto.set_trim | 校准单个舵机位置 | **servo_type**: 舵机类型(left_leg/right_leg/left_foot/right_foot)<br>**trim_value**: 微调值(-50到50度) |
...
// 左转2步
{"name": "self.otto.action", "arguments": {"action": "turn", "steps": 2}}
...
`servo_type` 可选值：`left_leg` / `right_leg` / `left_foot` / `right_foot`，`trim_value` 范围 `-50` ~ `50`
