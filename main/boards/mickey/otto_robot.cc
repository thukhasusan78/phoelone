        ...
        for (size_t i = 0; i < sizeof(camera_addresses); i++) {
            ...
            if (ret == ESP_OK) {
                ...
                if (ret == ESP_OK && (data[0] != 0 || data[1] != 0)) {
                    ...
                    ESP_LOGI(TAG, "Detected camera (OV2640 method) PID=0x%04X (addr=0x%02X)",
                             detected_pid, addr);
                    camera_found = true;
...
private:
    void InitializeWebSocketControlServer() {
        ...
        // Also broadcast MCP responses to WebSocket clients on port 8080
    ...
    bool InitializeCamera() {
        ...
        try {
            // Release I2C used during detection so it does not conflict with esp_camera init.
            i2c_del_master_bus(i2c_bus_);
            i2c_bus_ = nullptr;
            // Stop detection-phase XCLK so esp_camera can own the clock.
            ledc_stop(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL, 0);
...
public:
    OttoRobot()
        ...
        // Auto-detect hardware version (and camera type)
        has_camera_ = DetectHardwareVersion();
        ESP_LOGI(TAG, "Auto-detected hardware: %s", has_camera_ ? "camera" : "no-camera");
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_CAMERA
        // Force camera hardware config, but still detect the camera type
        has_camera_ = DetectHardwareVersion();
        if (!has_camera_) {
            // Detection failed; keep camera config with unknown camera type
            has_camera_ = true;
            camera_type_ = OTTO_CAMERA_UNKNOWN;
            ESP_LOGW(TAG, "Forcing camera config, but camera type was not detected");
            // Initialize I2C bus for the camera
            i2c_master_bus_config_t i2c_bus_cfg = {
                .i2c_port = I2C_NUM_0,
                .sda_io_num = CAMERA_VERSION_CONFIG.i2c_sda_pin,
                .scl_io_num = CAMERA_VERSION_CONFIG.i2c_scl_pin,
            ...
            ESP_LOGI(TAG, "Forcing camera hardware config");
        }
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_NO_CAMERA
        // Force no-camera hardware config
        has_camera_ = false;
        camera_type_ = OTTO_CAMERA_NONE;
        ESP_LOGI(TAG, "Forcing no-camera hardware config");
#else
#error \
    "Invalid OTTO_HARDWARE_VERSION; use OTTO_VERSION_AUTO, OTTO_VERSION_CAMERA, or OTTO_VERSION_NO_CAMERA"
#endif
